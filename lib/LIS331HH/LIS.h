...
protected:
    ...
    inline uint8_t readReg(const byte addr, byte &val) {
        ...
        if (Wire.endTransmission(false) != 0) return 1; // error
        if (Wire.requestFrom(i2cAddress, (uint8_t)1) != 1) return 2; // error
        val = Wire.read();                   // read 1 byte
        return 0; // success
    }
